                ...
                <div class="modal-body">
                    ...
                    <div>
                        ...
                        <textarea class="form-control" name="observacao_final" rows="3"
                            placeholder="Obrigatória ao concluir (Atendido)"></textarea>
                    </div>
                </div>
...
async function carregarAtendimentos() {
    try {
        ...
        tbody.innerHTML = atendimentos.map(a => {
            ...
            const tipo = labelRegistro(a, 'tipo_atendimento_nome', 'tipo_nome', 'tipo_atendimento', 'nome_tipo');
            const responsavel = labelRegistro(a, 'usuario_nome', 'responsavel', 'usuario', 'nome_usuario');
            const data = labelRegistro(a, 'data_atendimento', 'data');
            const status = a.status || '';
            const classeStatus = status === 'Atendido'
                ? 'text-bg-success'
            ...
            return `
                <tr>
                    ...
                    <td><span class="badge ${classeStatus}">${AtendeLabApi.escape(status)}</span></td>
                    <td class="text-end">
...
function abrirStatus(id, status) {
    ...
    document.querySelector('#formStatus [name="status"]').value = status || 'Aguardando';
